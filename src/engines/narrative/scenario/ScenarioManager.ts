...
export class ScenarioManager {
    ...
    private dialogueLogger: DialogueLogger | null = null;
    ...
    // Editor mode: event steps pause instead of auto-advancing
    ...
    constructor(
        ...
        this.dialogueLogger = dialogueLogger || null;
    ...
    exitFastForward(): void {
        if (!this.fastForwardEnabled) return;
        this.fastForwardEnabled = false;
        console.log('[Scenario] Fast-forward disabled');
    }
    ...
    getSteps(): ReadonlyArray<ScenarioStep> {
        return this.currentSequence?.steps ?? [];
    }
    ...
    goToStep(index: number): void {
        ...
        this.uiState = 'idle';
        console.log(`[Scenario] goToStep(${index})`);
    }

    /**
     * 현재 스텝을 재생 (playCurrentStep의 public wrapper)
     */
    ...
    replaceSequence(seq: ScenarioSequence): void {
        ...
        console.log(`[Scenario] replaceSequence: ${seq.name}`);
    }

    /**
     * 에디터 모드 설정.
     * true이면 이벤트 스텝에서 자동 전진하지 않고 멈춤.
     */
    setEditorMode(enabled: boolean): void {
        this.editorMode = enabled;
        console.log(`[Scenario] editorMode = ${enabled}`);
    }
    ...
    private handleTextStep(step: NarrationStep | DialogueStep): void {
        ...
        // 로그에 캡처
        this.dialogueLogger?.capture(step.text, speaker, step.type, this.currentIndex);
    ...
    private handleAutoStep(step: AutoStep): void {
        ...
        if (step.text) {
            ...
            // 로그에 캡처
            this.dialogueLogger?.capture(step.text, step.speaker, 'auto', this.currentIndex);
        }
    ...
    /**
     * 완전 스킵: 시나리오를 즉시 종료 (다이얼로그 숨김 포함)
     */
    skipToEnd(): void {
        if (!this.currentSequence) return;
        console.log('[Scenario] skipToEnd — forcing sequence end');
        this.endSequence();
    }
